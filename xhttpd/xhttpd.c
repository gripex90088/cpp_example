...
#define SERVER_URL "http://39.105.217.28" // "http://www.itcast.cn/"
...
int main(int argc, char **argv)
{
	...
	*/
    ...
    //测试有无请求之文件(或目录)，stat为获取指定目录位置文件file函数，sb为传出参数。
    if (stat(file, &sb) < 0)
...
static void file_infos(char *dir, char *name)
{
    ...
    if (lstat(path, &sb) < 0)
        //设置显示格式,"-"表示左对齐,不加"-"即为右对齐;-32表示显示内容占32列不足用空格补齐;32s表输出字符串的32个字符.
